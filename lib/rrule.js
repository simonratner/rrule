...
;(function (root, factory) {
  ...
  RRule.prototype = {
     ...
     */
    duration: function () {
      return this.options.dtend - this.options.dtstart
    },

    /**
     * Returns true iff the given date-time is included within the duration of
     * any instance of this rule.
     */
    includes: function (dt) {
      var prev = this.before(dt)
      if (!prev) {
        return false
      }
      if (this.options.until && dt > this.options.until) {
        return false
      }
      return dt >= prev && dt < (prev.valueOf() + this.duration())
    },
    ...
    for (i = 0; i < attrs.length; i++) {
      ...
      switch (key) {
        ...
        case 'DTSTART':
          ...
          break
        case 'DTEND':
          options.dtend = dateutil.untilStringToDate(value)
          break
        case 'UNTIL':
  ...
  RRuleStr.prototype = {
    ...
    _parseRfc: function (s, options) {
      ...
      var exrulevals = []
      var exdatevals = []
      var name, value, parts, parms, parm, dtstart, dtend, rset, j, k, datestrs, datestr
      ...
      if (!options.forceset && lines.length === 1 && (s.indexOf(':') === -1 ||
        ...
        for (i = 0; i < lines.length; i++) {
          ...
          if (name === 'RRULE') {
            ...
            dtend = dateutil.untilStringToDate(value)
          } else {
  ...
  RRuleStr.prototype._handle_DTEND = function (rrkwargs, name, value, options) {
    rrkwargs[name.toLowerCase()] = dateutil.untilStringToDate(value)
  }
